...
- **Подробности:**
  ...
  Функция, которая возвращает объект данных для экземпляра компонента. Не рекомендуем ослеживать в `data` объекты со своим собственным поведением, например, объекты API браузера или свойства прототипа. Лучше представлять данные компонента в виде простого объекта.
  ...
  После создания экземпляра доступ к оригинальном объекту с данными можно получить через `vm.$data`. Экземпляр компонента проксирует все свойства в объекте данных, поэтому обращение к `vm.a` будет эквивалентно `vm.$data.a`.
...
- **Подробности:**
  ...
  - `type`: может быть одним из нативных конструкторов: `String`, `Number`, `Boolean`, `Array`, `Object`, `Date`, `Function`, `Symbol`, или любой функцией пользовательского конструктора, или массивом из вышеперечисленных вариантов. Проверяет, что входной параметр имеет заданный тип, и выдаёт предупреждение, если тип не соответствует заданному. Подробнее в разделе [указания типа входных параметров](../guide/component-props.md#указание-типа-входных-параметров).

  - `default`: `any`
    Определяет значение по умолчанию для входного параметра. Если входной параметр не был передан, то будет использовано это значение. Объект или массив как значение по умолчанию необходимо возвращать из функции-фабрики.
  ...
  - `validator`: `Function`
    Пользовательская функция валидации, которая получает значение входного параметра единственным аргументом. В не-production окружении будет выводиться предупреждение в консоли, если вернётся значение, приводимое к false (то есть если валидация не пройдена). Подробнее в разделе [валидации входных параметров](../guide/component-props.md#валидация-входных-параметров).
...
- **Подробности:**
  ...
  Вычисляемые свойства кэшируются и пересчитываются только при изменении своих реактивных зависимостей. Обратите внимание, если какая-либо зависимость находится за пределами области видимости экземпляра (не является реактивной), то в таком случае вычисляемое свойство **обновляться не будет**.
...
- **Подробности:**
  ...
  Список/хэш пользовательских событий, которые могут генерироваться компонентом. Может использоваться массив (простой синтаксис) или объект (альтернативный синтаксис). Объект позволяет определить валидацию для событий.
...
- **Использование:**
  ...
  :::tip Примечание
  События, перечисленные в опции `emits`, **не наследуются** корневым элементом компонента и будут исключены из свойства `$attrs`.
  :::
