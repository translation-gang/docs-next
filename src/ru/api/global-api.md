...
- **Подробности:**
  ...
  Дочерние узлы, созданные с помощью h () или с использованием строк, для получения «текстовых VNode» или объекта со слотами. Опционально.
...
Передавать можно любое количество объектов, при этом приоритет будет у свойств, идущих позднее. Слушатели событий будут обработаны специальным образом, как `class` или `style`: значения этих свойств объединяются, а не перезаписываются.
