...
:::warning Примечание
Все хуки жизненного цикла автоматически привязывают свой контекст `this` к экземпляру и есть доступ к локальному состоянию, вычисляемым свойствам и методам. Поэтому **нельзя использовать стрелочные функции при определении хука жизненного цикла** (например, `created: () => this.fetchTodos()`). Причина в том, что стрелочные функции привязываются к родительскому контексту, поэтому `this` не будет экземпляром компонента и `this.fetchTodos` будет неопределён.
:::
...
- **Подробности:**
  ...
  Вызывается непосредственно перед начало монтирования: функция `render` будет вызвана в первый раз.
...
- **Подробности:**
  ...
  Обратите внимание, что `mounted` **не гарантирует**, что все дочерние компоненты уже будут примонтированы. Если необходимо подождать пока не будут отрисованы все дочерние, то можно воспользоваться [vm.$nextTick](../api/instance-methods.md#nexttick) внутри `mounted`:
...
- **Подробности:**
  ...
  Обратите внимание, что `updated` **не гарантирует**, что все дочерние компонента также были обновлены. Если необходимо подождать пока все не будут повторно отрисованы, можно воспользоваться [vm.$nextTick](../api/instance-methods.md#nexttick) внутри `updated`:
...
- **Подробности:**
  ...
  Вызывается при перехвате ошибки из любого дочернего компонента. Хук получает три аргумента: ошибку, экземпляр компонента, в котором произошла ошибка, и строку с информацией где была перехвачена ошибка. Хук может вернуть `false`, чтобы остановить дальнейшее распространение ошибки.

  :::tip Совет
  В этом хуке можно изменять состояние компонента. Однако важно, чтобы в шаблоне или render-функции были условия, которые оборачивают и отделяют другое содержимое при обнаружении ошибки; в противном случае компонент может попасть в бесконечный цикл перерисовки.
  :::
