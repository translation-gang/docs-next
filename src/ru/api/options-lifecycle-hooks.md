...
- **Подробности:**
  ...
  Вызывается при срабатывании перерисовки виртуального DOM. Подобно [`renderTracked`](#rendertracked), в качестве аргумента получает `debugger event`. Это событие сообщает, какая операция вызвала перерисовку, целевой объект и ключ этой операции.
