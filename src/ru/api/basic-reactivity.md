...
the default deep reactive/readonly conversion 

API `markRaw` и shallowXXX (см.ниже) позволяют выборочно отказаться от глубокого реактивного/только для чтения преобразования по умолчанию. В результате можно встраивать исходные, не-проксированные объекты в диаграмму состояния. Причины, по которым это используется:
...
Пропуск преобразования – продвинутая техника, которая доступна только на корневом уровне. Если установить вложенный, не-отмеченный исходный объект в реактивный объект и получить к нему доступ, то вернётся его проксированная версия. Это может привести к **опасностям идентификации**. Это операция, которая основывается на идентификации объекта, но использует как его исходную, так и проксированную версии:
...
Опасности идентификации встречаются редко. Однако чтобы безопасно использовать API `markRaw` и shallowXXX, нужно ясно понимать, как работает система реактивности.
:::
