...
Возвращает `true`, если прокси создавалась с помощью [`readonly`](#readonly), но оборачивается другим прокси, созданным с помощью [`reactive`](#reactive).
...
API `markRaw` и shallowXXX (см.ниже) позволяют выборочно отказаться от глубокого реактивного/только для чтения преобразования по умолчанию и встраивать исходные, не-проксированные объекты в диаграмму состояния. Причины, по которым это используется:
...
Опасность идентификации встречается редко. Однако чтобы безопасно использовать API `markRaw` и shallowXXX, нужно твёрдо понимать, как работает система реактивности.
:::

## `shallowReactive`

Создаёт реактивную прокси, которая отслеживает реактивность собственных свойств. Не выполняет глубокое реактивное преобразование вложенных объектов, предоставляя доступ к сырым значениям.
