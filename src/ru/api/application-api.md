...
Так как метод `createApp` возвращает экземпляр приложения, теперь можно составлять цепочки вызовов других методов. Информацию об этих методах вы найдёте в следующих разделах.
...
app.directive('my-directive', {
  ...
  created() {},
  // вызывается перед монтированием связанного родительского компонента
  beforeMount() {},
  // вызывается, когда связанный родительский компонент смонтирован
  mounted() {},
  // вызывается перед обновлением VNode, содержащего компонент
  beforeUpdate() {},
  // вызывается после обновления VNode, содержащего компонент, и VNodes его дочерних элементов
  updated() {},
  // вызывается перед тем, как будет размонтирован связанный родительский компонент
  beforeUnmount() {},
  // вызывается, когда связанный родительский компонент размонтирован
  unmounted() {}
})
...
// геттер, получить определение зарегистрированной директивы
...
- **Использование:**
  ...
  Заменяет `innerHTML` в предоставленном DOM-элементе на отрисованный шаблон корневого компонента приложения.
