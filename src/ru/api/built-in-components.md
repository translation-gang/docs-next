...
При использовании <component :is="vnode"> и передаче vnode того же типа, необходимо указать ключи:

```html
<component :is="current" :key="selected" />
```

В противном случае будут переданы рендереру две скомпилированных vnode одного типа. Поскольку они скомпилированы как полностью статические, то они не будут обновляться вообще.
