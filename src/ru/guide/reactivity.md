...
Чтобы получить возможность пересчитывать сумму всякий раз, когда значения изменятся, первое что потребуется — обернуть её в функцию:
...
Vue отслеживает какая функция в данный момент была запущена с помощью *эффекта*. Эффект — обёртка вокруг функции, которая начинает отслеживание непосредственно перед её вызовом. Таким образом Vue знает, какой эффект выполняется в любой заданной точке и может запустить его снова при необходимости.

Чтобы понять это лучше, давайте попробуем реализовать нечто подобное самостоятельно, без Vue, чтобы посмотреть как это может работать.
...
Для этого требуется `createEffect`, чтобы отслеживать выполнение функции вычисления суммы. Реализовать подобное можно примерно так:
...
Запуск эффекта будет добавлять его в массив `runningEffects` перед вызовом `fn`. Теперь, если потребуется узнать какой эффект запущен в данный момент, достаточно будет проверить этот массив.
...
Но понимание какой код выполняется — лишь одна из частей головоломки. Как же Vue узнает, какие значения использует эффект и как определяет что они изменились?

## Как Vue отслеживает изменения

Отследить переназначение локальных переменных в предыдущих примерах не получится, такого механизма попросту нет в JavaScript. Можно лишь отслеживать изменения свойств объектов.

Поэтому, когда из функции `data` компонента возвращается простой объект JavaScript, Vue обернёт его в [Proxy](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Proxy) с обработчиками для `get` и `set`. Прокси были представлены в ES6 и позволяют Vue 3 избавиться от ограничений системы реактивности, которые существовали в предыдущих версиях Vue. 
...
Это достаточно поверхностное объяснение, которое требует для понимания некоторых знаний о [Proxy](https://developer.mozilla.org/ru/docs/Web/JavaScript/Reference/Global_Objects/Proxy). Давайте немного углубимся. Есть много обучающих материалов о Proxy, но самое важное что нужно знать это то, что **Proxy — объект, который содержит в себе другой объект или функцию и позволяет «перехватывать» их.**

Использовать можно вот так: `new Proxy(target, handler)`
...
При использовании Proxy есть одна трудность — привязка к `this`. Хочется, чтобы любой метод был привязан к Proxy, а не к отслеживаемому объекту, чтобы была возможность перехватывать и их тоже. К счастью, ES6 предоставляет ещё одну возможность, названную `Reflect`, которая позволяет избежать этой проблемы с минимальными усилиями:
...
3. **Перезапускать код, который считывал значения изначально**: функция `trigger` ищет какие эффекты зависят от свойства и запускает их.

Проксируемый объект невидим для пользователя, но под капотом он позволяет Vue отслеживать зависимости и уведомлять о считывании свойств или их изменениях. Единственным ограничением таких объектов в том, что проксированные объекты при выводе в консоль форматируются немного иначе, поэтому лучше установить [vue-devtools](https://github.com/vuejs/vue-devtools) чтобы отслеживать их состояние в более удобном интерфейсе.
...
Начиная с Vue 3, реактивность теперь доступна из [отдельного пакета](https://github.com/vuejs/vue-next/tree/master/packages/reactivity). Функция, которая оборачивает `$data` в прокси называется [`reactive`](../api/basic-reactivity.md#reactive). Её можно вызвать напрямую, если нужно обернуть объект в реактивный прокси без необходимости использовать компонент:
...
Возможности, предоставляемые пакетом реактивности, будут рассмотрены подробнее в следующих разделах. Там будут рассмотрены функции `reactive` и `watchEffect`, с которыми уже повстречались, а также способы использования других функций реактивности, таких как `computed` и `watch`, без необходимости создания компонента.
...
Обратите внимание, что Vue не оборачивает в Proxy примитивные значения (числа или строки), поэтому всё ещё можно использовать `===` непосредственно с этими значениями:
