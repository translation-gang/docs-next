...
Or, if you’ve been unit-testing an application involving [async components](../component-dynamic-async.md), chances are you’ve written something like this:
...
:::warning Важно
The above only applies to the [ES Modules builds](../installation.md#explanation-of-different-builds) for use with tree-shaking capable bundlers - the UMD build still includes all features and exposes everything on the Vue global variable (and the compiler will produce appropriate output to use APIs off the global instead of importing).
:::
