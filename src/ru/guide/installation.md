...
Когда в имени пользователя присутствует пробел, например «Mike Baker», то может случиться, что vite не сможет успешно выполниться. В таком случае используйте команду:
...
В [папке `dist/` npm-пакета](https://cdn.jsdelivr.net/npm/vue@3.0.2/dist/) можно обнаружить несколько различных сборок Vue.js. Рассмотрим подробнее какой файл из `dist` и для каких случаев должен использоваться.
