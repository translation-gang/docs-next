...
Синтаксис двойных фигурных скобок не будет работать с HTML-атрибутами. Вместо него следует использовать [директиву `v-bind`](../api/directives.md#v-bind):
...
В случаях с булевыми атрибутами, где само их наличие уже подразумевает значение `true`, `v-bind` работает немного по-другому. Например:

```html
<button v-bind:disabled="isButtonDisabled">Кнопка</button>
```

Атрибут `disabled` будет добавляться если значение `isButtonDisabled` истинно. Он также будет добавлен если значением будет пустая строка для сохранения консистентности с `<button disabled="">`. При использовании других ложных значений атрибут будет опущен.
