...
Подобный вид записи несколько отличается от обычного HTML-кода, но в именах атрибутов допускаются символы `:` и `@`, а браузеры, поддерживающие Vue, могут их корректно обработать. Кроме того, в итоговой разметке их уже не будет. Сокращённый синтаксис полностью опционален, но вероятно вы оцените его лаконичность и удобство.
