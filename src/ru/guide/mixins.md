...
Например, каждая примесь может иметь свою собственную функцию `data`. Каждая из них будет вызвана, а возвращённые объекты объединены. Свойства из `data` компонента при этом будут иметь приоритет в случаев конфликтов.
